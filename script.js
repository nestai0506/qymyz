...
const PRODUCTS = [
  ...
  { id:2, vessel:"saba", price:1200,
    name:{ kz:"Ащылау қымыз", ru:"Кисловатый кумыс", en:"Tangy kumis" },
  ...
  { id:3, vessel:"smallkese", price:600,
    name:{ kz:"Жұмсақ қымыз", ru:"Мягкий кумыс", en:"Mild kumis" },
    ...
    tag:{ kz:"Балаларға", ru:"Детям", en:"For kids" }, vol:{ kz:"0.5 л", ru:"0.5 л", en:"0.5 L" } },
  { id:4, vessel:"kubi", price:3600,
  ...
  { id:5, vessel:"torsyk", price:1700,
    name:{ kz:"Саумал", ru:"Саумал", en:"Saumal" },
    desc:{ kz:"Жаңа сауылған бие сүті — ашытылмаған, нәзік әрі майда дәмді.", ru:"Свежевыдоенное кобылье молоко — незаквашенное, с нежным мягким вкусом.", en:"Freshly milked mare's milk — unfermented, delicate and mild." },
    tag:{ kz:"Жаңа", ru:"Свежий", en:"Fresh" }, vol:{ kz:"1 л", ru:"1 л", en:"1 L" } },
  { id:6, vessel:"ornate", price:2000,
...
const I18N = {
  ...
  stat_24:         { kz:"24 сағ", ru:"24 ч", en:"24 hrs" },
  ...
  added_ok:        { kz:"Қосылды", ru:"Готово", en:"Added" },
  ...
  about_lead:      { kz:"Біздің шаруашылық Жетісу облысы, Талдықорған қаласында орналасқан. Биелерімізді таза жайылымда, дәрі-дәрмексіз өсіреміз әрі қымызды ата-бабадан келе жатқан дәстүрлі әдіспен, сабада ашытамыз.", ru:"Наше хозяйство расположено в Жетысуской области, город Талдыкорган. Кобылиц мы растим на чистых пастбищах, без лекарств, а кумыс заквашиваем в сабе по традициям предков.", en:"Our farm is located in the Zhetysu region, city of Taldykorgan. We raise our mares on clean pastures, without medicines, and ferment the kumis in a saba following our ancestors' traditions." },
  ...
  c_addr_v:        { kz:"Талдықорған қаласы,<br>дача Ащыбұлақ", ru:"г. Талдыкорган,<br>дачный массив Ащыбулак", en:"Taldykorgan,<br>Ashybulak dacha area" },
  ...
  cart_checkout:   { kz:"WhatsApp-пен тапсырыс беру", ru:"Заказать через WhatsApp", en:"Order via WhatsApp" },
  cart_note:       { kz:"Тапсырыс WhatsApp арқылы жіберіледі", ru:"Заказ отправится через WhatsApp", en:"Your order is sent via WhatsApp" },
  cart_empty:      { kz:"Кесе әлі бос.<br>Қымыз таңдаңыз", ru:"Пиала пока пуста.<br>Выберите кумыс", en:"The bowl is still empty.<br>Choose your kumis" },
  cart_vol:        { kz:"Көлемі:", ru:"Объём:", en:"Volume:" },

  toast_added:     { kz:" себетке қосылды", ru:" добавлен в корзину", en:" added to cart" },
  // Қате хабары кешірім сұрамайды — не істеу керегін айтады
  toast_empty:     { kz:"Алдымен қымыз таңдаңыз", ru:"Сначала выберите кумыс", en:"Choose a kumis first" },
  wa_intro:        { kz:"Сәлеметсіз бе! Мен осы қымыздарға тапсырыс бергім келеді:", ru:"Здравствуйте! Я хочу заказать этот кумыс:", en:"Hello! I would like to order this kumis:" },
...
function addToCart(id, sourceEl) {
  ...
  bumpCartIcon();
  if (sourceEl) {
    flyDrop(sourceEl);        // SIGNATURE: тамшы себетке ұшады
    markButtonAdded(sourceEl); // батырма «Құйылды ✓» болады
  }
  showToast(`«${product.name[LANG]}»${t("toast_added")}`);
}

/* ============================================================
   SIGNATURE — «құйылу» метафорасы
   Тамшы бөтелкеден себетке ұшады, себет иконкасы толады.
   ============================================================ */
const REDUCED_MOTION = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

function flyDrop(sourceEl) {
  if (REDUCED_MOTION) return;
  const card = sourceEl.closest(".card");
  const from = (card ? card.querySelector(".card__vessel") : sourceEl).getBoundingClientRect();
  const to = cartBtn.getBoundingClientRect();

  const drop = document.createElement("span");
  drop.className = "drop-fly";
  document.body.appendChild(drop);

  const x0 = from.left + from.width / 2, y0 = from.top + from.height / 2;
  const x1 = to.left + to.width / 2,     y1 = to.top + to.height / 2;

  drop.style.left = `${x0 - 7}px`;
  drop.style.top  = `${y0 - 7}px`;

  // доға сызып ұшады (ортасында жоғары көтеріледі)
  const anim = drop.animate([
    { transform: "translate(0,0) scale(1)",  opacity: 1 },
    { transform: `translate(${(x1-x0)*0.5}px, ${(y1-y0)*0.5 - 70}px) scale(1.25)`, opacity: 1, offset: 0.55 },
    { transform: `translate(${x1-x0}px, ${y1-y0}px) scale(.35)`, opacity: 0 },
  ], { duration: 720, easing: "cubic-bezier(.4,.1,.5,1)" });

  anim.onfinish = () => drop.remove();
}

/* Себет иконкасының толуы: 6 данада толық болады */
function updateCartFill() {
  const el = document.getElementById("cartLiquid");
  if (!el) return;
  el.style.setProperty("--fill", Math.min(getCount() / 6, 1).toFixed(2));
}

/* Батырманың 3-ші күйі: «Құйылды ✓» (1.2 сек) */
function markButtonAdded(btn) {
  if (btn.dataset.busy) return;
  btn.dataset.busy = "1";
  const original = btn.textContent;
  btn.textContent = `${t("added_ok")} ✓`;
...
function renderCart() {
  ...
  updateCartFill();   // себет иконкасының қымыз деңгейі

  if (cart.length === 0) {
    // Бос күй: ою-өрнекті бос кесе (себет иконкасы емес — бренд тілінде)
    cartBody.innerHTML = `
      <div class="cart__empty">
        <svg viewBox="0 0 120 100" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round">
          <ellipse cx="60" cy="34" rx="42" ry="10"/>
          <path d="M18 34 C20 58 36 76 60 76 C84 76 100 58 102 34"/>
          <path d="M27 45 q7 5 14 0 q7 -5 14 0 q7 5 14 0 q7 -5 14 0" stroke-opacity=".55" stroke-width="2"/>
          <path d="M48 76 L50 88 Q60 93 70 88 L72 76"/>
          <ellipse cx="60" cy="89" rx="11" ry="3.5"/>
        </svg>
        <p>${t("cart_empty")}</p>
      </div>`;
...
/* ============================================================
   СКРОЛЛ REVEAL — элементтер көрінген сайын жұмсақ шығады
   ============================================================ */
function initReveal() {
  if (REDUCED_MOTION || !("IntersectionObserver" in window)) return;

  const targets = document.querySelectorAll(
    ".section__head, .card, .about__media, .about__content, .quote .container, .contact__card, .footer__inner"
  );
  const io = new IntersectionObserver((entries) => {
    entries.forEach((e) => {
      if (!e.isIntersecting) return;
      // карточкалар кезекпен шығады (60ms аралықпен)
  ...
  targets.forEach((el) => {
    ...
    if (parent === group) { i++; } else { group = parent; i = 0; }
    el.dataset.revealDelay = String(Math.min(i, 5) * 60);
    io.observe(el);
  });
}
...
initScene3D();
renderCart();
initReveal();     // скроллда жұмсақ шығу (карточкалар рендерленген соң)
